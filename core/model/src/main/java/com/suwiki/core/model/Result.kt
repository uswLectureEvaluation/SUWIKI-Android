...
sealed class Result<out T> {
    ...
    fun getOrThrow() =
        if (isSuccessful) {
            ...
            throw IllegalStateException(
                "Cannot be called under Failure conditions.",
            )
        }
    ...
    fun errorOrThrow() =
        if (isFailure) {
            ...
            throw IllegalStateException(
                "Cannot be called under Success conditions.",
            )
        }
    ...
    fun <R> map(transform: (T) -> R): Result<R> {
        return if (isSuccessful) Success(transform(getOrThrow())) else (this as com.suwiki.core.model.Result.Failure)
    }
}

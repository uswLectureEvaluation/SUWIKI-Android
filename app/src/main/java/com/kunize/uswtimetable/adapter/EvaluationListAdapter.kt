...
class EvaluationListAdapter : RecyclerView.Adapter<EvaluationListAdapter.Holder>() {
    ...
    RecyclerView.ViewHolder(binding.root) {
        fun setData(data: EvaluationData) {
            binding.lectureName.text = data.name
            binding.averRatingBar.rating = data.aver
            binding.lectureProfessor.text = data.professor
            binding.lectureType.text = data.type
            binding.averScore.text = data.aver.toString()
        }

...
class MyPostRepository(private val dataSource: MyPostRemoteDataSource) {
    suspend fun getEvaluation(page: Int) = withContext(Dispatchers.IO) { dataSource.getMyEvaluations(page) }
    ...
    suspend fun deleteExamInfo(id: Long) = withContext(Dispatchers.IO) { dataSource.deleteMyExamInfo(id) }
